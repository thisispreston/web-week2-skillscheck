...
const total = myNumbers.reduce((acc, el) => acc + el)
...
myNumbers.forEach((num, index) => {myNumbersIndex.push(index)}, 0)
...
const forTheLoveOfGeorge = notGeorge.map(el => {
	if (el !== 'George') {
		return 'George'
	}
})
...
const enemies = people.filter(el => {
	if (el.friend === false) {
		return el}
})
...
const totallyAwesome = people.reduce((acc, el) => acc + el.awesomeLevel, 0)
